...
async function main() {
    ...
    app.get('/api/dogs', async (req, res)=> {
        try{
            const[rows] = await connection.query('SELECT Dogs.name AS Dog_name, Dogs.size, Users.username AS owner_username FROM Dogs JOIN Users ON Dogs.owner_id = Users.user_id;');
            res.json(rows);
        }
        catch(err){
